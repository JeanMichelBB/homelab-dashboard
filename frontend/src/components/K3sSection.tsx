  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-gray-800 rounded-xl p-5 bg-gray-900">
        <div className="text-xs font-mono text-gray-500 mb-5">k3s cluster</div>
      ...
      {k3s && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {(['total', 'running', 'pending', 'failed'] as const).map(key => (
            <div key={key} className="border border-gray-800 bg-gray-900 rounded-lg px-4 py-3 text-center">
              <div className={`text-2xl font-bold ${key === 'failed' && k3s.pods.failed > 0 ? 'text-red-400' : key === 'running' ? 'text-green-400' : 'text-white'}`}>
                {k3s.pods[key]}
              </div>
              <div className="text-xs text-gray-500 mt-1 font-mono">{key}</div>
            </div>
          ))}
        </div>
      )}
...
function K3sNode({ name, role, online, hw }: {
  ...
  return (
    <div className={`border rounded-lg px-4 py-3 min-w-52 ${isControl ? 'border-blue-900 bg-blue-950/30' : 'border-gray-800 bg-gray-950'}`}>
      ...
      <div className={`text-xs px-1.5 py-0.5 rounded inline-block mb-2 ${isControl ? 'bg-blue-900/50 text-blue-300' : 'bg-gray-800 text-gray-500'}`}>
        {role}
      </div>
      <div className="text-xs text-gray-600">{hw}</div>
    </div>
  )
}
